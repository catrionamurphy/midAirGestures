...
class swipeListener(Leap.Listener):
    ...
    def on_frame(self, controller):
        ...
        for hand in frame.hands:
            ...
            start_coords = (self.frameList[0].palm_position.x, self.frameList[0].palm_position.y, self.frameList[0].palm_position.z)
            end_coords = (self.frameList[-1].palm_position.x, self.frameList[-1].palm_position.y, self.frameList[-1].palm_position.z)
            
            """
            print "start coords: " + str(start_coords)
            print "end coords: " + str(end_coords)
            """

            majorAxis = self.getMajorAxis(0, absY, 0)

            if majorAxis == 'y' and absY > 50:
                if difference > 0:
